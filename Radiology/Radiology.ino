...
//
//    *** THIS IS THE Radiology Version
//
//    Version 2.0 -- Adjustment for Hiplink.  Added 10 second delay after HELO in mail send.
//    Version 1.07.1 -- Added all Locations AND OLED support.
...
byte mac[] = { 0x90, 0xA2, 0xDA, 0x0F, 0x88, 0x69 };
...
String locations[] = {"ICU_Pod_A", "ICU_Pod_B", "ICU_Pod_C", "3_East_Pod_A", "3_East_Pod_B", "3_East_Pod_C", "4_East_Pod_A", "4_East_Pod_B",
                      "4_East_Pod_C", "5_North", "5_South", "5_Nursery", "4_North", "4_South", "3_North", "3_South", "OPS", "Surgery", "ED_Green_Zone", "ED_Red_Zone", 
                      "ED_Clean_Supply", "MEC", "Outpatient_Infusion", "Radiology", "Wound_Care", "Cardiology", "PACU"
                     };
String recipients[] = {"Freshloc_Pharm@phnpage.net", "commtest@phnpage.net", "TrashOnUnit@phnpage.net", "Laundry_Full@phnpage.net",
...
locations [1]  = ICU Pod B          recipients[1] =  commtest@phnpage.net            subjects[1] = Trash Emptied			bodies[1] = Trash Emptied
...
lo/cations [10] = 5 South            recipients[10] =
...
void setup()
{
  ...
  display.println("Trash/Laundry I/F\nDesigned & Written by\nDrew Dickenson 2014");
  display.display();
  delay(5000);
  display.clearDisplay();
  display.setTextSize(1);
...
//    Serial.println("Pin 19 is High");
  ...
  if (needTrashPickedUpButtonStateA == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[23], subjects[0], bodies[0] ))
    {
  ...
  else  if (trashHasBeenPickedUpButtonStateA == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[23], subjects[1], bodies[1] ))
    {
      digitalWrite(dirtyUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedA, HIGH); // If the sendEmail() Function returns 1
  ...
  if (needLaundryPickedUpButtonStateA == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[3], locations[23], subjects[2], bodies[2] ))
    {
      digitalWrite(dirtyUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedA, HIGH); // If the sendEmail() Function returns 1
  ...
  else if (laundryHasBeenPickedUpButtonStateA == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[4], locations[23], subjects[3], bodies[3] ))
    {
      digitalWrite(dirtyUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedA, HIGH); // If the sendEmail() Function returns 1
  ...
  if (needFreshLinensButtonStateA == HIGH)
  {
    ...
    if (cleanUtilityErrorStatus = sendEmail(recipients[5], locations[23], subjects[4], bodies[4] ))
    {
  ...
  if (freshLinensDeliveredButtonStateA == HIGH)
  {
    ...
    if (cleanUtilityErrorStatus = sendEmail(recipients[6], locations[23], subjects[5], bodies[5] ))
    {
      digitalWrite(cleanUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(cleanUtilityRedA, HIGH); // If the sendEmail() Function returns 1
...
/*
  ...
  if (needTrashPickedUpButtonStateB == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[26], subjects[0], bodies[0] ))
    {
  ...
  else  if (trashHasBeenPickedUpButtonStateB == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[26], subjects[1], bodies[1] ))
    {
      digitalWrite(dirtyUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedB, HIGH); // If the sendEmail() Function returns 1
  ...
  if (needLaundryPickedUpButtonStateB == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[3], locations[26], subjects[2], bodies[2] ))
    {
      digitalWrite(dirtyUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedB, HIGH); // If the sendEmail() Function returns 1
  ...
  else if (laundryHasBeenPickedUpButtonStateB == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[4], locations[26], subjects[3], bodies[3] ))
    {
      digitalWrite(dirtyUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedB, HIGH); // If the sendEmail() Function returns 1
  ...
  if (needFreshLinensButtonStateB == HIGH)
  {
    ...
    if (cleanUtilityErrorStatus = sendEmail(recipients[5], locations[1], subjects[4], bodies[4] ))
    {
  ...
  if (freshLinensDeliveredButtonStateB == HIGH)
  {
    ...
    if (cleanUtilityErrorStatus = sendEmail(recipients[6], locations[1], subjects[5], bodies[5] ))
    {
      digitalWrite(cleanUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(cleanUtilityRedB, HIGH); // If the sendEmail() Function returns 1
...
/*
  if (needTrashPickedUpButtonStateC == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[21], subjects[0], bodies[0] ))
    {
  ...
  else  if (trashHasBeenPickedUpButtonStateC == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[21], subjects[1], bodies[1] ))
    {
      digitalWrite(dirtyUtilityGreenC, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedC, HIGH); // If the sendEmail() Function returns 1
  ...
  if (needLaundryPickedUpButtonStateC == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[3], locations[21], subjects[2], bodies[2] ))
    {
      digitalWrite(dirtyUtilityGreenC, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedC, HIGH); // If the sendEmail() Function returns 1
  ...
  else if (laundryHasBeenPickedUpButtonStateC == HIGH)
  {
    ...
    if (dirtyUtilityErrorStatus = sendEmail(recipients[4], locations[21], subjects[3], bodies[3] ))
    {
      digitalWrite(dirtyUtilityGreenC, LOW); //Turn the GREEN led off to indicate end of sending;
      delay(WAIT);
      digitalWrite(dirtyUtilityRedC, HIGH); // If the sendEmail() Function returns 1
...
//Print IP to the display

  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(0,0);
  display.println("Radiology\n");
  display.println("Your IP address is: ");
  for (byteIPByte = 0; byteIPByte < 4; byteIPByte++)
  {
    // Print the value of each byte of the IP address
    display.print(Ethernet.localIP()[byteIPByte], DEC);
    display.print(".");
  }
  display.display();
