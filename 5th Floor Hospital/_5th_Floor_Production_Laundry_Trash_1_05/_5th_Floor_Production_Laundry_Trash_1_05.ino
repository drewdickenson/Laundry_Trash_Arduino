...
//    Version 1.06 -- This Version - Final 1/16/2014
//    Version 1.05 -- Enable Nursery Location
...
byte mac[] = { 0x00, 0x02, 0xB3, 0xDA, 0xD0, 0x01 };
...
String locations[] = {"ICU_Pod_A", "ICU_Pod_B", "ICU_Pod_C", "3_East_Pod_A", "3_East_Pod_B", "3_East_Pod_C", "4_East_Pod_A", "4_East_Pod_B",
                      "4_East_Pod_C", "5_North", "5_South", "5_Nursery", "4_North", "4_South", "3_North", "3_South", "OPS", "Surgery", "ED", "MEC"};
String recipients[] = {"Freshloc_Pharm@phnpage.net","ddickenson@phnpage.net","TrashOnUnit@phnpage.net","Laundry_Full@phnpage.net",
                       "Laundry_Emptied@phnpage.net","Linens_Request@phnpage.net","Linens_Delivered@phnpage.net", "5TrashOnUnit@phnpage.net" };
...
locations [0]  = ICU Pod A          recipients[0] =  Freshloc_Pharm@phnpage.net        subjects[0] = Trash Full
locations [1]  = ICU Pod B          recipients[1] =  ddickenson@phnpage.net            subjects[1] = Trash Emptied
locations [2]  = ICU Pod C          recipients[2] =  TrashOnUnit@phnpage.net           subjects[2] = Laundry Full
locations [3]  = 3 East Pod A       recipients[3] =  Laundry_Full@phnpage.net          subjects[3] = Laundry Emptied
locations [4]  = 3 East Pod B       recipients[4] =  Laundry_Emptied@phnpage.net       subjects[4] = Linens Needed
locations [5]  = 3 East Pod C       recipients[5] =  Linens_Request@phnpage.net        subjects[5] = Linens Stocked
locations [6]  = 4 East Pod A       recipients[6] =  Linens_Delivered@phnpage.net
locations [7]  = 4 East Pod B       recipients[7] =  5TrashOnUnit@phnpage.net
...
{
  Serial.begin(115200);
  Serial.println("5th FLOOR HOSPITAL\n");
...
void loop() 
{
    ...
    if (needTrashPickedUpButtonStateA == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[9], subjects[0], bodies[0] ))
      { 
  ...
  else  if (trashHasBeenPickedUpButtonStateA == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[9], subjects[1], bodies[1] ))
      { 
        digitalWrite(dirtyUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedA, HIGH); // If the sendEmail() Function returns 1 
   ...
   if (needLaundryPickedUpButtonStateA == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[3], locations[9], subjects[2], bodies[2] ))
      { 
        digitalWrite(dirtyUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedA, HIGH); // If the sendEmail() Function returns 1 
  ...
  else if (laundryHasBeenPickedUpButtonStateA == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[4], locations[9], subjects[3], bodies[3] ))
      { 
        digitalWrite(dirtyUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedA, HIGH); // If the sendEmail() Function returns 1 
   ...
   if (needFreshLinensButtonStateA == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (cleanUtilityErrorStatus = sendEmail(recipients[5], locations[9], subjects[4], bodies[4] ))
      { 
   ...
   if (freshLinensDeliveredButtonStateA == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (cleanUtilityErrorStatus = sendEmail(recipients[6], locations[9], subjects[5], bodies[5] ))
      { 
        digitalWrite(cleanUtilityGreenA, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(cleanUtilityRedA, HIGH); // If the sendEmail() Function returns 1 
    ...
    if (needTrashPickedUpButtonStateB == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[10], subjects[0], bodies[0] ))
      { 
  ...
  else  if (trashHasBeenPickedUpButtonStateB == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[10], subjects[1], bodies[1] ))
      { 
        digitalWrite(dirtyUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedB, HIGH); // If the sendEmail() Function returns 1 
   ...
   if (needLaundryPickedUpButtonStateB == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[3], locations[10], subjects[2], bodies[2] ))
      { 
        digitalWrite(dirtyUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedB, HIGH); // If the sendEmail() Function returns 1 
  ...
  else if (laundryHasBeenPickedUpButtonStateB == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[4], locations[10], subjects[3], bodies[3] ))
      { 
        digitalWrite(dirtyUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedB, HIGH); // If the sendEmail() Function returns 1 
   ...
   if (needFreshLinensButtonStateB == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (cleanUtilityErrorStatus = sendEmail(recipients[5], locations[10], subjects[4], bodies[4] ))
      { 
   ...
   if (freshLinensDeliveredButtonStateB == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (cleanUtilityErrorStatus = sendEmail(recipients[6], locations[10], subjects[5], bodies[5] ))
      { 
        digitalWrite(cleanUtilityGreenB, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(cleanUtilityRedB, HIGH); // If the sendEmail() Function returns 1 
    ...
    if (needTrashPickedUpButtonStateC == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[11], subjects[0], bodies[0] ))
      { 
  ...
  else  if (trashHasBeenPickedUpButtonStateC == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[2], locations[11], subjects[1], bodies[1] ))
      { 
        digitalWrite(dirtyUtilityGreenC, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedC, HIGH); // If the sendEmail() Function returns 1 
   ...
   if (needLaundryPickedUpButtonStateC == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[3], locations[11], subjects[2], bodies[2] ))
      { 
        digitalWrite(dirtyUtilityGreenC, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedC, HIGH); // If the sendEmail() Function returns 1 
  ...
  else if (laundryHasBeenPickedUpButtonStateC == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (dirtyUtilityErrorStatus = sendEmail(recipients[4], locations[11], subjects[3], bodies[3] ))
      { 
        digitalWrite(dirtyUtilityGreenC, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(dirtyUtilityRedC, HIGH); // If the sendEmail() Function returns 1 
   ...
   if (needFreshLinensButtonStateC == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (cleanUtilityErrorStatus = sendEmail(recipients[5], locations[11], subjects[4], bodies[4] ))
      { 
   ...
   if (freshLinensDeliveredButtonStateC == HIGH)
  {
    Serial.println("Trying to Send Email");
      ...
      if (cleanUtilityErrorStatus = sendEmail(recipients[6], locations[11], subjects[5], bodies[5] ))
      { 
        digitalWrite(cleanUtilityGreenC, LOW); //Turn the GREEN led off to indicate end of sending;
        delay(WAIT);
        digitalWrite(cleanUtilityRedC, HIGH); // If the sendEmail() Function returns 1 
